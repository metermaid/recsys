...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    private SparseVector ddd(long user) {
    ...
    private SparseVector makeUserVector(long user) {
        ...
        double mean = getMean(userRatings);
        ...
        for (Rating r: userRatings) {
            ...
            if (p != null) {
                ...
                MutableSparseVector weightedItemVector = iv.mutableCopy();
                weightedItemVector.multiply(p.getValue() - mean);
    ...
    private double getMean(List<Rating> ratings){
        double sum = 0;
